...
- [ ] 1.4 手动回归：Alt+Space 弹速记条、Alt+N 弹主窗口、托盘左键与右键菜单、关主窗口进程留存、第二实例聚焦已有实例
- [ ] 1.5 回归延迟测量，反复触发 15 次以上，确认 p95 仍在 200ms 内（拆分前基线 27ms）
...
- [ ] 2.8 验证：空目录选为 vault 后结构补齐；已有 vault 重开不覆盖任何文件；删掉 inbox.md 重启后自动重建
...
- [ ] 6.1 加 `blake3` 依赖，实现文件内容哈希
- [ ] 6.2 入队时记录 base_hash
- [ ] 6.3 实现 `replace_line` 快速路径：哈希匹配则按行号替换，只改该行、其余字节不变
- [ ] 6.4 实现慢速路径：哈希不匹配时全文扫描定位 `old_content`
- [ ] 6.5 慢速路径匹配到多行时**拒绝并标为失败**，不猜测（见 design D4）
- [ ] 6.6 目标行找不到时拒绝并标为失败
- [ ] 6.7 实现 `create` 操作：目标已存在则失败
- [ ] 6.8 实现整文件替换操作（供后续版本恢复使用）
- [ ] 6.9 单元测试：三种操作 × 基线匹配/不匹配/目标消失/多行同内容

## 7. 重试与失败可见性

- [ ] 7.1 实现重试：失败且 retries < 3 时延迟后重试并递增计数
- [ ] 7.2 重试耗尽：保留队列记录并标记失败，emit 失败事件
- [ ] 7.3 新增 command：查询失败列表（文件路径、操作类型、失败原因）
- [ ] 7.4 新增 command：重试指定失败项、丢弃指定失败项
- [ ] 7.5 主窗口加失败列表 UI，含重试与丢弃两个操作
- [ ] 7.6 验证：把 vault 目录设为只读后速记，看到失败提示；恢复权限后点重试成功写入

## 8. 崩溃恢复

- [ ] 8.1 启动时先消费 `write_queue` 中的遗留记录，再接受新请求
- [ ] 8.2 实现 append 的重放保护：队列记录存 `applied_marker`，恢复时先比对文件末尾内容再决定是否重放（见 design 的 Risks，append 是唯一不幂等的操作）
- [ ] 8.3 验证：写入过程中强杀进程，重启后队列被正确消费，inbox.md 无重复行也无丢失行

## 9. 收尾

- [ ] 9.1 `cargo clippy` 无警告
- [ ] 9.2 `npm run build` 与 `cargo build` 均通过
- [ ] 9.3 全量手动回归：第 1.4 项的全部条目 + 速记写入 + 失败恢复
- [ ] 9.4 最终延迟测量，记录 p95 数字
- [ ] 9.5 更新 README 的「明确还没做的」一节，移除已完成的 actor 条目

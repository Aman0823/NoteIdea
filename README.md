...
- **待办语法解析器（3.2-3.4）**：Rust 唯一实现，行尾右向左扫描元数据区（`@时间 !重复 #标签 ^强度 ~id`），引号屏蔽字面量，非法值降级为正文并记录位置。时间求值与解析分离（当前时刻作为参数），日期算术不依赖 chrono。
- **待办身份与 ID 分配（3.6）**：随机 4-8 位十六进制短哈希，冲突扩位，全库查重。分配后立即经 actor 写回 md，写盘失败则操作整体失败。
- **待办索引（3.7）**：启动时后台全量扫描 vault 的 md 文件，解析待办并写入 `todos` 表。单文件失败不中断扫描。索引可随时丢弃重建，md 是唯一真源。暴露扫描进度状态（Scanning/Ready/Failed）与手动重扫入口。

## 明确还没做的

骨架期刻意留白，避免在没验证性能前堆代码：

- 编辑器（CM6 + decoration）、输入辅助弹层、提醒引擎、便签窗口、git 集成——尚未开始。
- `replace_line` / `create` / 整文件替换的核心逻辑与测试已就位，但还没暴露成 command——眼下没有调用方（等便签/编辑器）。
- `allocate_and_writeback` 已实现但还没有调用方——等设提醒/贴屏功能接入时才会触发 ID 分配。
- 索引已在后台扫描，但前端还没有消费索引数据的界面（聚合视图、标签过滤等下一期做）。
- `capture` 无条件加 `- [ ]` 前缀，正式版应由语法解析决定是否加。
